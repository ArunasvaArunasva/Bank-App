...
public class CheckingAccount extends Account{
		...
		if((amount>balance)){ 	
			...
			if (customer instanceof Senior){
				...
				if((customer.isVIP())){
					...
					fees=5;
						...
						balance = balance-(amount+10+0.01);
						fees = 10.01;
				...
				balance = balance - (amount + 25+0.03);
				fees = 25.03;
			}
			...
			for(int i=0; i<transactions.length;i++)
			{
				if(transactions[i]==null)
				{
					...
					transactions[i]= new Transaction((byte)1, amount, new Date(), fees ,"Withdraw from checking account "+amount+"$");
					super.size++;
				}
			}
			}else{
			...
			for(int i=0; i<transactions.length;i++)
			{
				if(transactions[i]==null)
				{
					...
					transactions[i]= new Transaction((byte)1, amount, new Date(), 0 ,"Withdraw from checking account "+amount+"$");
					super.size++;
				}
			}}else{
				reallocate();
	...
	balance = balance + (balance*customer.getCheckInterest());
	if(super.size<transactions.length){
		for(int i = 0; i<transactions.length;i++){
			if(transactions[i]==null){ 
	...
	//returns string represenation of account
	public String toString(){
		return(" Customer name: " + this.getCustomerName()+ " Balance: " + this.getBalance() + " accountNumber: " + this.getaccountNumber() + " " + "Account Type: Checking");
	}
